...
def _init_tl_cnn(
    ...
    assert model_path.exists(), f"[TL-CNN] Model not found: {model_path}"
    assert mapping_path.exists(), f"[TL-CNN] Mapping not found: {mapping_path}"
...
def map_to_bdd_category(name: str) -> str:
    ...
    if n in {"bicycle"}: return "bicycle"        # 给 rider 计数备用
