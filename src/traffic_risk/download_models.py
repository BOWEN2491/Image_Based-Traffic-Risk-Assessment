"""Download and verify the public model assets described by the package manifest."""
...
def load_manifest(path: Path | None = None) -> ModelManifest:
    """Load the one canonical package manifest and validate its contract."""
    manifest_path = path or Path(str(MANIFEST))
    return ModelManifest.from_file(manifest_path)


def download(destination_root: Path, force: bool = False, *, manifest_path: Path | None = None) -> None:
    manifest = load_manifest(manifest_path)
    if manifest.is_withdrawn():
        raise RuntimeError(
            "The packaged model release is withdrawn and cannot be downloaded. "
            "Use a supported release or train a local model bundle."
        )
    if manifest.distribution_status != "public":
        raise RuntimeError("Only public model manifests can be downloaded")
    for asset in manifest.assets:
        if not asset.destination:
            raise RuntimeError(f"Manifest asset has no destination: {asset.name or '<unnamed>'}")
        ...
        try:
            ...
            if partial.stat().st_size != asset.size or sha256(partial) != asset.sha256:
                raise RuntimeError(f"Downloaded asset failed verification: {asset.name or asset.destination}")
